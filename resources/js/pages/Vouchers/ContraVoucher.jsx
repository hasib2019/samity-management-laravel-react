...
const ContraVoucher = () => {
  ...
  // Separate states for Debit and Credit GL selection
  const [drGlQuery, setDrGlQuery] = useState('');
  const [crGlQuery, setCrGlQuery] = useState('');
  ...
  const submit = async (e) => {
    e.preventDefault();
    if (form.dr_gl_id === form.cr_gl_id) {
        Swal.fire({ icon: 'error', title: 'Debit and Credit GL cannot be same', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
        return;
    }
    try {
      const payload = {
        ...
        dr_gl_id: form.dr_gl_id,
        cr_gl_id: form.cr_gl_id,
        samity_id: form.samity_id,
        naration: form.naration,
      };
      await api.post('/contra-voucher', payload);
      Swal.fire({ position: 'top-end', icon: 'success', title: 'Contra posted', showConfirmButton: false, timer: 1200, toast: true });
      setForm({ 
          tran_date: new Date().toISOString().slice(0, 10), 
          amount: '', 
          dr_gl_id: '', 
          cr_gl_id: '',
          samity_id: form.samity_id, // Keep samity selected
          naration: '' 
      });
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800">Contra Voucher</h2>
      </div>
        ...
        <form onSubmit={submit}>
          <div className="px-6 py-5">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              {/* Debit GL */}
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700">Debit Account (Receiver)</label>
                <Combobox
                  value={glAccounts.find(a => a.id === form.dr_gl_id) || null}
                  onChange={(val) => setForm(prev => ({ ...prev, dr_gl_id: val?.id || '' }))}
                >
                  <div className="relative">
                    <Combobox.Input
                      className="px-3 py-2 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                      displayValue={(val) => val ? `${val.glac_code} - ${val.glac_name}` : ''}
                      onChange={(e) => setDrGlQuery(e.target.value)}
                    />
                    <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white shadow-lg ring-1 ring-black/5">
                      {getFilteredGl(drGlQuery).map(gl => (
                        <Combobox.Option
                          key={gl.id}
                          value={gl}
                          className="cursor-pointer select-none px-3 py-2 text-sm hover:bg-gray-100"
              ...
              {/* Credit GL */}
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700">Credit Account (Source/Giver)</label>
                <Combobox
                  value={glAccounts.find(a => a.id === form.cr_gl_id) || null}
                  onChange={(val) => setForm(prev => ({ ...prev, cr_gl_id: val?.id || '' }))}
                >
                  <div className="relative">
                    <Combobox.Input
                      className="px-3 py-2 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                      displayValue={(val) => val ? `${val.glac_code} - ${val.glac_name}` : ''}
                      onChange={(e) => setCrGlQuery(e.target.value)}
                    />
                    <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white shadow-lg ring-1 ring-black/5">
                      {getFilteredGl(crGlQuery).map(gl => (
                        <Combobox.Option
                          key={gl.id}
                          value={gl}
                          className="cursor-pointer select-none px-3 py-2 text-sm hover:bg-gray-100"

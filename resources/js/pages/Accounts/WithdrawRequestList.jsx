...
const WithdrawRequestList = () => {
    ...
    const fetchRequests = async () => {
        try {
            const response = await api.get('/withdraw-requests?status=pending');
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Failed to fetch withdraw requests',
            });
        } finally {
            setLoading(false);
        }
    ...
    const handleStatusChange = async (id, newStatus) => {
        ...
        try {
            ...
            const result = await Swal.fire({
                title: 'Are you sure?',
                text: `You are about to ${actionText} this withdraw request.${newStatus === 'approved' ? ' This will create a transaction.' : ''}`,
                icon: 'warning',
            ...
            if (result.isConfirmed) {
                await api.put(`/withdraw-requests/${id}`, { status: newStatus });
                
                Swal.fire(
                    'Updated!',
                    `Withdraw request has been ${newStatus}.`,
                    'success'
                );
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            <h2 className="text-2xl font-bold mb-6">Pending Withdraw Requests</h2>
                        ...
                        {requests.length === 0 ? (
                            ...
                            requests.map((request) => (
                                <tr key={request.id}>
                                    ...
                                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        {request.debit_account?.account_no}
                                    </td>
                                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        {request.amount}
                                    </td>
